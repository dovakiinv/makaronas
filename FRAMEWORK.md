...
- **English prompts, Lithuanian responses.** All system prompts and instructions to the Trickster (and any other AI role) are written in English — this maximises the model's reasoning depth and instruction-following fidelity. The AI's student-facing output is always in Lithuanian, enforced by a language instruction layer in the context assembly pipeline. Prompt files in `prompts/` are English. Cartridge `trickster_content` and `trickster_opening` fields that reach the student are Lithuanian. Never mix: prompts stay English, student-visible text stays Lithuanian.
